...
st.set_page_config(page_title="NHS Schematic (Clickable Background)", layout="wide")
...
def init_state():
    ...
    st.session_state.setdefault("uirevision", "keep")
init_state()
...
def make_figure(img: Image.Image, markers: List[Marker]) -> go.Figure:
    ...
    # 1) Transparent heatmap covering the entire image area to capture clicks ANYWHERE
    x = np.linspace(0, w, 50)  # coarse grid is fine; keeps payload small
    y = np.linspace(0, h, 50)
    z = np.zeros((y.size, x.size))
    fig.add_trace(go.Heatmap(
        x=x, y=y, z=z,
        showscale=False, opacity=0.0, hoverinfo="skip",
        name="click-layer"
    ))
    ...
    fig.update_layout(
        ...
        uirevision=st.session_state.uirevision,
    )
    return fig
...
st.write("Click anywhere on the image to drop a marker. Drag to pan, scroll to zoom.")

events = plotly_events(
    fig,
    click_event=True, hover_event=False, select_event=False,
    override_height=min(900, st.session_state.img_h),
    key="plotly_clicks_bg"
)

# Debug (optional)
# st.write("Last event:", events[-1] if events else None)
...
if st.session_state.markers:
    rows = [asdict(m) | {"x_px": int(m.x*st.session_state.img_w), "y_px": int(m.y*st.session_state.img_h)} for m in st.session_state.markers]
